...
export async function POST(req) {
    ...
    try {
        ...
        try {
            ...
            // Fallback 1: gemini-3.6-flash with search grounding (typically has separate/larger limits)
            try {
                console.log("[NeuralInvestigator] Attempting Fallback 1: gemini-3.6-flash with search...");
                const model = genAI.getGenerativeModel({ 
                ...
                steps.push({ msg: 'Usando motor de búsqueda secundario (Gemini 3.6)', icon: 'Globe', source: 'google' });
            } catch (err1) {
                ...
                try {
                    ...
                    // Fallback 3: gemini-3.6-flash without search grounding (highly stable free tier model)
                    console.log("[NeuralInvestigator] Attempting Fallback 3: gemini-3.6-flash without search...");
        ...
        try {
            ...
            console.warn("[NeuralInvestigator] Primary structuring failed, falling back to gemini-3.6-flash:", structError.message);
            const fallbackStruct = genAI.getGenerativeModel({ model: "gemini-3.6-flash" });
