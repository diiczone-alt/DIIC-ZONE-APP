...
export default function FilmmakerGuideReaderPage({ params }) {
    const { id } = use(params);
    ...
    const guide = FILMMAKER_GUIDES.find(g => g.id === id) || FILMMAKER_GUIDES[0];

    return (
        <div className="flex-1 flex flex-col font-sans bg-[#050511] text-white overflow-y-auto selection:bg-cyan-500/30 min-h-screen custom-scrollbar">
            {/* Progress Bar */}
            <motion.div 
                className="fixed top-0 left-0 right-0 h-1.5 bg-cyan-500 origin-left z-50 shadow-[0_0_10px_rgba(6,182,212,0.5)]"
                style={{ scaleX }}
            />
            ...
            <header className="sticky top-0 h-20 border-b border-white/5 bg-[#050511]/80 backdrop-blur-xl flex items-center justify-between px-8 z-40">
                <div className="flex items-center gap-6">
                    ...
                    <div>
                        <h1 className="text-sm font-black uppercase tracking-widest text-cyan-400/50">{guide.type}</h1>
                ...
                <div className="flex items-center gap-3">
                    ...
                    {!read ? (
                        <button 
                            ...
                            className="ml-4 px-6 py-3 bg-cyan-600 hover:bg-cyan-500 text-white text-xs font-black uppercase tracking-widest rounded-xl transition-all shadow-lg shadow-cyan-600/20"
                        >
                            Marcar como Leído
                        </button>
                    ) : (
            ...
            <div className="max-w-4xl mx-auto w-full pt-16 px-8">
                ...
                <p className="text-lg md:text-xl text-gray-400 leading-relaxed font-medium mb-16 max-w-2xl">
                    {guide.description}
                </p>
                ...
                <div className="grid grid-cols-12 gap-12">
                    ...
                    <div className="col-span-12 lg:col-span-8 space-y-12">
                        {guide.content.map((section, i) => (
                            <section key={i} className="space-y-6">
                                <h3 className="text-2xl font-black text-white flex items-center gap-4 group">
                                    <span className="text-cyan-500 font-mono text-sm opacity-50">0{i+1}</span>
                                    <span className="uppercase tracking-tight group-hover:text-cyan-400 transition-colors">{section.title}</span>
                                </h3>
                        ...
                        {!read && (
                            <div className="pt-20">
                                <button 
                                    onClick={() => setRead(true)}
                                    className="w-full py-8 bg-[#0E0E18] border border-white/10 rounded-[32px] hover:border-cyan-500/50 hover:bg-white/[0.02] transition-all group flex flex-col items-center justify-center gap-4"
                                >
                                    <div className="w-16 h-16 rounded-full bg-cyan-600 flex items-center justify-center text-white shadow-2xl group-hover:scale-110 transition-transform">
                    ...
                    <aside className="hidden lg:block lg:col-span-4 sticky top-32 h-fit space-y-8">
                        <div className="bg-[#0E0E18] border border-white/10 rounded-3xl p-6">
                            <h4 className="text-[10px] font-black text-cyan-400 uppercase tracking-[0.2em] mb-4">Sobre este recurso</h4>
                            <div className="space-y-4">
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className="text-xs font-bold text-white">Jun 2026</span>
                                </div>
                            </div>
                        </div>

                        <div className="bg-gradient-to-br from-cyan-600/10 to-transparent border border-cyan-500/20 rounded-3xl p-6">
                            <h4 className="text-[10px] font-black text-white uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                                <BookOpen className="w-3 h-3 text-cyan-400" /> Guía Próxima
                            </h4>
                            <p className="text-sm font-bold text-white mb-2">Planificación & Scouting Técnico</p>
                            <button className="text-xs font-black text-cyan-400 hover:text-white transition-colors flex items-center gap-2">
