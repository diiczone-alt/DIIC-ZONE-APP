...
export default function FilmmakerSidebar() {
    ...
    const menuItems = [
        { icon: Clapperboard, label: 'Producciones', path: '/workstation/filmmaker' },
        { icon: Calendar, label: 'Agenda de Rodaje', path: '/workstation/filmmaker/calendar' },
        { icon: UploadCloud, label: 'Subidas', path: '/workstation/filmmaker/uploads' },
        { icon: FileText, label: 'Guías de Rodaje', path: '/workstation/filmmaker/guides' },
        { icon: MessageCircle, label: 'Mensajes', path: '/workstation/filmmaker/messages' },
        { icon: Clock, label: 'Tiempos', path: '/workstation/filmmaker/times' },
        { icon: Star, label: 'Reputación', path: '/workstation/filmmaker/reputation' },
        { icon: GraduationCap, label: 'Academia', path: '/workstation/filmmaker/academy' },
        ...
        >
            ...
            <div className="h-20 flex items-center justify-center border-b border-white/5">
                ...
                {!collapsed && (
                    ...
                    >
                        <span className="block font-bold text-white whitespace-nowrap">Filmmaker Pro</span>
            ...
            <nav className="flex-1 py-6 px-3 space-y-2 overflow-y-auto custom-scrollbar">
                {menuItems.map((item, index) => {
                    const isActive = pathname === item.path || (item.path !== '/workstation/filmmaker' && pathname.startsWith(item.path));
                    return (
                        <Link key={index} href={item.path}>
                            <div className={`
                                flex items-center p-3 rounded-xl transition-all duration-200 group relative
                                ${isActive ? 'bg-cyan-600/10 text-cyan-400' : 'text-gray-400 hover:text-white hover:bg-white/5'}
                                ${collapsed ? 'justify-center' : ''}
                            `}>
                                <item.icon className={`w-5 h-5 shrink-0 ${isActive ? 'text-cyan-400' : 'group-hover:text-white'}`} />
                ...
                {!collapsed && (
                    <div className="px-3 mt-4 mb-2">
                        <div className="bg-white/5 rounded-xl p-3 border border-white/5">
                            <CapacityBar current={65} max={100} label="Mi Carga" mini />
                        </div>
                    </div>
                )}
            </nav>
