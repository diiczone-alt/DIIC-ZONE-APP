...
function renderContent(tab, selectedClient, setSelectedClient, setActiveTab, clients = [], loading = false, clientTasks = [], loadingTasks = false, user = null, squad = [], globalTasks = [], notifications = [], loadingNotifications = false, handleMarkAsRead, searchParams, onProfileUpdate) {
    ...
    switch (tab) {
        ...
        case 'calendar': return <UnifiedCalendar role="cm" clientId={selectedClient?.id} client={selectedClient} clients={clients || []} squad={squad || []} user={user} />;
            ...
            if (!data.success) {
                if (data.connected === false) {
                    toast.info("Cuenta de Meta no conectada", {
                        id: 'meta-sync',
                        description: "Inicia sesión con Facebook para vincular la cuenta y sincronizar datos reales."
                    });
