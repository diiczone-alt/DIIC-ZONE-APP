...
import { 
    ChevronDown, User, Settings, LogOut, CreditCard, Layout
} from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { useAuth } from '@/context/AuthContext';
import Link from 'next/link';

export default function WorkstationProfileDropdown({ role = 'Audio' }) {
                        ...
                        >
                            ...
                            <div className="space-y-1">
                                {(() => {
                                    ...
                                    const panelRoute = lowerRole.includes('audio') 
                                        ? '/workstation/audio'
                                        : (lowerRole.includes('film') || lowerRole === 'filmmaker') 
                                            ? '/workstation/filmmaker' 
                                            : (lowerRole.includes('design') || lowerRole === 'designer') 
                                                ? '/workstation/designer' 
                                                : '/workstation/editor';
                                                
                                    const financeRoute = lowerRole.includes('audio')
                                        ? '/workstation/finance'
                                        : (lowerRole.includes('film') || lowerRole === 'filmmaker') 
                                            ? '/workstation/finance' 
