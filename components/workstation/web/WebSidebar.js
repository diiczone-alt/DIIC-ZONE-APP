...
import {
    Globe, Layout, Code, Server,
    CheckCircle, MessageSquare, Play, Settings, LogOut,
...
export default function WebSidebar() {
    ...
    const clientItems = [
        { icon: Globe, label: 'Nuevo Sitio', path: '/workstation/web' },
        { icon: Layout, label: 'Mis Proyectos', path: '/workstation/web/my-projects' },
    ];

    const developerItems = [
        { icon: Layout, label: 'Dashboard', path: '/workstation/web/dashboard' },
        { icon: Code, label: 'Proyectos Activos', path: '/workstation/web/projects' },
        { icon: MessageSquare, label: 'Tickets / Cambios', path: '/workstation/web/tickets' },
        { icon: Server, label: 'Hosting & Dominios', path: '/workstation/web/hosting' },
    ];

    const menuItems = mode === 'developer' ? developerItems : clientItems;
    ...
    return (
        ...
        >
            ...
            <div className="h-20 flex items-center justify-center border-b border-white/5">
                <div className="w-10 h-10 rounded-xl bg-cyan-600 flex items-center justify-center font-bold text-white shadow-lg shadow-cyan-500/20">
                ...
                {!collapsed && (
                    ...
                    >
                        <span className="block font-bold text-white whitespace-nowrap">Web Dev</span>
                        <span className="text-[10px] text-gray-400 uppercase tracking-wider">{mode === 'developer' ? 'Dev Console' : 'Client Studio'}</span>
                    </motion.div>
                )}
            </div>
            ...
            <nav className="flex-1 py-6 px-3 space-y-2 overflow-y-auto custom-scrollbar">
                {menuItems.map((item, index) => {
                    const isSelected = (item.path === '/workstation/web' || item.path === '/workstation/web/dashboard')
                        ? pathname === item.path
                    ...
                    return (
                        <Link key={index} href={item.path}>
                            <div className={`
                                ...
                                ${isSelected ? 'bg-cyan-600/10 text-cyan-400' : 'text-gray-400 hover:text-white hover:bg-white/5'}
                                ${collapsed ? 'justify-center' : ''}
                            `}>
                                <item.icon className={`w-5 h-5 shrink-0 ${isSelected ? 'text-cyan-400' : 'group-hover:text-white'}`} />
