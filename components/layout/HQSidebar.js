...
export default function HQSidebar() {
    ...
    return (
        <aside className="fixed left-0 top-0 h-screen w-64 bg-[#08081a] border-r border-white/10 flex flex-col z-[100] lg:z-50 shadow-2xl">
            ...
            <div className={`h-24 flex items-center justify-between border-b border-white/10 transition-all duration-300 px-6 ${isCollapsed ? 'lg:justify-center lg:px-0' : ''}`}>
                ...
                <button
                    ...
                    className="lg:hidden p-2 text-slate-400 hover:text-white bg-white/5 hover:bg-white/10 rounded-xl transition-all cursor-pointer"
                ...
                {menuItems.map((item) => {
                    ...
                    return (
                        <Link 
                            ...
                            onClick={() => setIsMobileOpen(false)}
                        >
                            <div className={`flex items-center rounded-xl transition-all duration-200 group relative px-4 py-3 ${isCollapsed ? 'lg:justify-center lg:p-3.5' : 'gap-3'} ${isActive
