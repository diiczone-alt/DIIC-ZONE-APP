...
let supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || 'https://pigojfotwzgahcmtvyko.supabase.co'
const supabaseAnonKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InBpZ29qZm90d3pnYWhjbXR2eWtvIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzU2OTQ5MDIsImV4cCI6MjA5MTI3MDkwMn0.d_tUGfzX7WUC8R31wrP7LBLyyrNcHY6igkkjs6hYEJ8'

let supabaseClient

// Use direct Supabase endpoint to bypass custom proxy routing/PATCH timeout issues
if (supabaseUrl === 'https://api.diiczone.com') {
    supabaseUrl = 'https://pigojfotwzgahcmtvyko.supabase.co';
}

const isRealCloud = true // Always connect to real cloud database using the configured endpoint
    ...
    const createChainableMock = (tableName, currentData = null, mutationInfo = null) => {
        const mock = {
            ...
            maybeSingle: async function () {
                await delay(100);
                return { data: currentData && currentData.length > 0 ? currentData[0] : null, error: null };
            },
            ...
            insert: function (item) {
                ...
                if (typeof window !== 'undefined') {
                    ...
                    itemsToAdd.forEach(it => {
                        // Impedir duplicados en tabla team por nombre o email
                        if (tableName === 'team') {
                            const nameExists = currentRecords.some(r => r.name && r.name.toLowerCase().trim() === (it.name || '').toLowerCase().trim());
                            const emailExists = it.email && currentRecords.some(r => r.email && r.email.toLowerCase().trim() === it.email.toLowerCase().trim());
                            if (nameExists || emailExists) {
                                console.warn(`[MockDB] Prevented duplicate team insert for name: ${it.name}, email: ${it.email}`);
                                return;
                            }
                        }
                        // Impedir duplicados en tabla profiles por nombre completo
                        if (tableName === 'profiles') {
                            const nameExists = currentRecords.some(r => r.full_name && r.full_name.toLowerCase().trim() === (it.full_name || '').toLowerCase().trim());
                            if (nameExists) {
                                console.warn(`[MockDB] Prevented duplicate profile insert for name: ${it.full_name}`);
                                return;
                            }
                        }
                        filteredItems.push({
                            id: it.id || (tableName === 'team' ? `TEAM-${Math.floor(1000 + Math.random() * 9000)}` : Math.floor(Math.random() * 900000) + 100000),
                            status: it.status || 'activo',
                            ...it
                        });
                    });
                    
                    if (filteredItems.length > 0) {
                        localStorage.setItem(`mock_db_${tableName}`, JSON.stringify([...currentRecords, ...filteredItems]));
                    }
                    return createChainableMock(tableName, filteredItems);
                }

                return createChainableMock(tableName, itemsToAdd);
            },
            delete: function () { 
            ...
            upsert: function (item) {
                ...
                if (typeof window !== 'undefined') {
                    ...
                    itemsToUpsert.forEach(newItem => {
                        const index = currentRecords.findIndex(r => {
                            if (newItem.id && r.id === newItem.id) return true;
                            if (newItem.email && r.email && r.email.toLowerCase() === newItem.email.toLowerCase()) return true;
                            return false;
                        });
                        if (index !== -1) {
